...
def fix_cli_newlines(args):
    """
    Iterate through all string attributes on an args object (for example,
    an argparse.Namespace instance) and replace the literal sequence "\\n"
    with an actual newline character.

    Args:
        args: Command-line argument object, such as argparse.Namespace.

    Returns:
        The modified args object.
    """

    # vars(args) returns a dictionary-style view of all attributes on the object.
    for key, value in vars(args).items():
        # 1. Only process string values.
        if isinstance(value, str):
            # 2. Replace the literal "\\n" with a real newline.
            # Checking first avoids unnecessary string work.
            if '\\n' in value:
                new_value = value.replace('\\n', '\n')
                # 3. Write the modified value back to the original object.
