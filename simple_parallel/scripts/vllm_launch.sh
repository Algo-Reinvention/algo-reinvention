...
do
    ...
    echo "Configuring vLLM server on GPUs [${CUDA_DEVICES_STR}] at port ${PORT}..."
    ...
    tmux send-keys -t $TARGET_WINDOW \
        "CUDA_VISIBLE_DEVICES=${CUDA_DEVICES_STR} python start_vllm_server.py \
...
echo ""
echo "Successfully launched ${SERVER_COUNT} vLLM server(s) in session '$SESSION_NAME'."
