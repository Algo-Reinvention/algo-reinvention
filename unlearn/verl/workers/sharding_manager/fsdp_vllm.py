...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def update_params(self, updated_params, peft_config=None):
        ...
        clean_params = {}
        logger.warning(f"Starting parameter cleanup. Total params before: {len(updated_params)}")
        for k, v in updated_params.items():
            if any(keyword in k for keyword in multimodal_keywords):
                continue
            new_key = k
        ...
        logger.warning(f"Parameter cleanup finished. Total params after: {len(clean_params)}")

        # debug log
        if len(clean_params) > 0:
            first_key = next(iter(clean_params))
            logger.warning(f"Sample clean key: {first_key}")
        updated_params = clean_params
        #/@jzhao
