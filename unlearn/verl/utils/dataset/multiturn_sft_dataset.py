...
class MultiTurnSFTDataset(Dataset):
    ...
    def __init__(self, parquet_files: str | list[str], tokenizer, config=None):
        self._debug_rank0_only = int(os.environ.get("RANK", "0")) == 0
        ...
        # NOTE @jzhao;  Convert OmegaConf ListConfig to regular list and ensure all elements are strings
    ...
    def __getitem__(self, item):
        ...
        try:
            ...
            processed_str = chat_str.replace("<think>\n\n</think>\n\n<think>\n", "<think>\n")
        ...
        if self._debug_rank0_only and is_worker0 and not self._has_saved_debug:
            self._debug_buffer.append(res)
            self._debug_count += 1
            if len(self._debug_buffer) >= 5:
                try:
                    self._save_debug_to_arrow()
                except Exception as e:
                    logging.error(f"Failed to save debug samples: {e}")

        return res

    def _save_debug_to_arrow(self):
        import os
        import pandas as pd
        from datetime import datetime
        
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        debug_root = os.getenv("VERL_SFT_DEBUG_DIR", "").strip()
        if not debug_root:
            project_root = os.getenv("PROJECT_ROOT", "").strip()
            if not project_root:
                raise RuntimeError("PROJECT_ROOT is empty. Please set PROJECT_ROOT before writing SFT debug outputs.")
            debug_root = str(Path(project_root) / "sft" / "saves" / "_debug")
        save_dir = os.path.join(debug_root, timestamp)
        os.makedirs(save_dir, exist_ok=True)
        
        processed_data = []
        for item in self._debug_buffer:
            entry = {
                k: v.tolist() if isinstance(v, torch.Tensor) else v 
                for k, v in item.items()
            }
            entry['decoded_input'] = self.tokenizer.decode(item['input_ids'], skip_special_tokens=False)
            processed_data.append(entry)
            
        df = pd.DataFrame(processed_data)
        save_path = os.path.join(save_dir, "first_5_samples.parquet")
        df.to_parquet(save_path, engine='pyarrow')
        
        logging.info(f">>> [DEBUG] Successfully saved first 5 samples to Arrow/Parquet: {save_path}")
        self._has_saved_debug = True
